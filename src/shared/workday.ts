...
/** Options for workday determination. */
export interface WorkdayOpts {
 ...
 * @param opts 工作日选项（如贵司月末周六规则开关）。
 */
export function getDayInfo(
  date: Date,
  overrides?: Record<number, YearHolidayData>,
  opts?: WorkdayOpts
): DayInfo {
  ...
  const companyRule = opts?.companyLastSaturday ?? true
  ...
  // 3. 贵司规则：月末最后一个周六（且非法定节假日）。可通过 opts 关闭。
  if (companyRule && isLastSaturdayOfMonth(date)) {
...
export function remainingWorkdays(
  ...
  while (d.getTime() <= end.getTime()) {
    if (getDayInfo(d, overrides, opts).isWorkday) count++
    d.setDate(d.getDate() + 1)
  }
  return count
}
