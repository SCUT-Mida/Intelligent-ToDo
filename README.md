...
- **内网选项（v1.25.1）**：⚙ 网络设置支持**直连（绕过系统代理）**、**忽略 TLS 证书校验**（自签名/企业代理证书）、超时 1–300 秒可调；错误卡片透出 `net::ERR_*` 原始错误码与原因分析，证书/代理类错误提供**一键开启对应开关并重发**
- **会话环境诊断（v1.25.3+）**：🔍 诊断一键探测 git/node 解析、仓库识别、PATH 重建、注册表与通用工具路径配置，报告可复制反馈
- **零账号**：所有收藏与历史存于本地 `api-tool.json`（原子写 + 损坏备份）

### 🛠️ 通用工程能力

- **通用工具路径（v1.25.5）**：设置 → 通用 → 工具路径，集中配置 git / node 绝对路径（含浏览与可用性验证），仓库导航扫描、Agent 终端环境（含 agent 内部 git/node 调用）、任务模式统一生效；配合**注册表 PATH 重建**（机器级 + 用户级），彻底解决打包应用 PATH 净化导致的「git/node 不是内部或外部命令」类问题——Agent 内嵌工具（启动钩子、MCP、白名单校验）不再受影响
